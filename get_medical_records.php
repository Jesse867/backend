...
// Query to get medical records, patient details, and medications for a specific patient
$query = "SELECT 
    mr.*,
    p.first_name,
    p.last_name,
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        if ($row['record_id'] != $currentRecordId) {
            if ($currentRecordId !== null) {
                $medicalRecords[] = $currentRecord;
            }
            $currentRecordId = $row['record_id'];
            $currentRecord = [
                "medicalRecord" => [
